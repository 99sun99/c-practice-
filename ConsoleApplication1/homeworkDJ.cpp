...
public:
	...
	int top();
	int size();
	int parent(int);// parent position
	int left(int);
	int right(int);
	void print();
	void swap(int&, int&);
...
Graph::Graph(bool directed, float density)
{
	Max_V = MAXV;
	...
	this->density = Max_E * density;
	cout << "The total numbers of nodes and edges are " << Max_V << " and " << Max_E << "." << endl;
	cout << "The density is " << density << ". " << (Max_E * density) << " edges are selected as edges." << endl;
...
void Graph::set_edge() {
	...
	while (k < this->density)
	{
		int x = rand() % 50;
		int y = rand() % 50;
		int weight = rand() % 10 + 1;//the path is from 1 to 10
...
ShortestPath::ShortestPath(float density)
{

	Dgraph = new Graph(false,  density);
	Dgraph->set_edge();
...

{
	...
	while (true)
	{
		...
		final_path.push_back(parent[end]);
		end = parent[end];
	}
	//cout << "test end" << parent[end] << " " << end << endl;
	for (int i = 0; i < final_path.size(); i++)
	{
		final_path1.push_back ( final_path.at(final_path.size()-i-1));
	}

	

	for (int i = 0; i < final_path1.size(); i++)
	{

		cout << final_path1.at(i) << " ";
	}
	cout << endl;
	for (int i = 0; i < MAXV; i++)
	{
		if (0 < distance[i]&&distance[i] < 11)
		{
			avarage+= distance[i];
			count++;
		}
	...
	cout << "the average path length is " << avarage << endl;
...
int main() {
	...
	density = 0.2;
	ShortestPath *Dtest = new ShortestPath(density);//randomly set the weight within the function set_edge.
	Dtest->shortest_path(begin);
	for (i = 0; i < 30; i++)//test 30 times from the start to different ends' path and average length.
	{
		srand(i);
		end = rand() % 50 + 1;

		cout << endl;
		cout << "The shortest path from node " << begin << " to node " << end << " is: ";
		Dtest->path(end);
		Dtest->path_size(end);
	}
	...
	density = 0.4;
	ShortestPath *Dtest1 = new ShortestPath(density);//randomly set the weight within the function set_edge.
	Dtest1->shortest_path(begin);
	//for (i = 0; i < 30; i++)//test 30 times from the start to different ends' path and average length.
	//{
	//	srand(i);
	//	end = rand() % 50 + 1;
